...
SpriteRenderer::SpriteRenderer() {
    s_rendererCount++;
    CreateQuad();
    if (!s_defaultTexture) CreateDefaultTexture();
}
...
void SpriteRenderer::Render() {
    if (!m_visible || !s_spriteShader) return;
...
void SpriteRenderer::CleanupShader() {
    if (s_rendererCount == 0) s_spriteShader.reset();
}

void SpriteRenderer::CreateDefaultTexture() {
    if (s_defaultTexture) return;
    try {
...
void SpriteRenderer::CleanupDefaultTexture() {
    if (s_rendererCount == 0) s_defaultTexture.reset();
}
